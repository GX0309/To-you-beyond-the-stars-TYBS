...
label gx_music_un_02:
    m 4eud "[player], 我学了一首新歌!"
    m 7hub "你想听听吗?"
    menu:
        "好啊!":
            $ mas_unlockEVL("gx_music_fts_again", "EVE")
            jump gx_music_fts

        "抱歉, 今天不行":
            m 1gksdrb "好吧..."
            m 7kua "等你有空了我再给你弹!"
            $ mas_unlockEVL("gx_music_fts_again", "EVE")       # 解锁第二个事件注意事件标签，应与下面一致
            return
...
init 5 python:
    addEvent(
        Event(
            persistent.event_database,
            eventlabel="gx_music_fts_again",           # 初始锁定（依赖第一个事件解锁）
            category=['音乐'],
            prompt="你能在为我唱一次from the start吗?",
            pool=True,
            unlocked=False,
...
label gx_music_fts_again:
        # 指定音乐文件修改路径
    $ music_file = "Submods/To You Beyond the Stars/TYBS_other/music/from the start.mp3" # 可以同时准备一个显示用的曲名,曲名不可重复
    $ song_display = "from the start"   # 定义音乐名称 也可以不写直接写在应该出现的对话里面就行了
